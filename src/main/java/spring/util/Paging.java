...
public class Paging {
	private int nowPage, //현재 페이지값
				...
				startPage, // 시작페이지 값
				endPage, // 마지막 페이지 값
				begin, // 한 페이지의 시작 레코드의 값
				end; // 한 페이지의 마지막 레코드의 값
	
	private boolean isPrePage; // 이전 기능 가능 여부(true일때 이전기능 활성화)
	private boolean isNextPage; // 다음 기능 가능 여부(true일때 다음기능 활성화)
	...
	public Paging(int nowPage, int rowTotal, int blockList, int blockPage) {
		...
		// 현재 페이지의 값이 전체페이지의 값보다 크다면 전체 페이지 값을 현재 페이지값으로 지정
		...
		// 현재 페이지 값에 의해 시작게시물의 행번호와 마지막 게시물의 행번호를 지정하여 현재 페이지에 보여질 게시물 목록을 얻을 준비
